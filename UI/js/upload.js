...
function foo(){	
    var str = document.getElementById('studentregno').value;
    var camIndex = document.getElementById('cameraIndex') ? document.getElementById('cameraIndex').value : 0;
    if(str.length>1){
        console.log("Entered Reg. No. is " + str);
        console.log("attempting file write");
        fs.writeFileSync('py/helper.txt', str);
        console.log("File written, calling python script");
        var python = require('child_process').spawn('python', ['py/capture.py', camIndex]);
        python.stdout.on('data',function(data){
            console.log("data: ",data.toString('utf8')+ " from Python ");
        });
    }
